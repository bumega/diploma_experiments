...
from gpu_model_aligned_rase import GpuRaseConfig, GpuRbfKernelClassifier
from gpu_shap_model_aligned_rase import GPU_CRITERIA_MAIN, GPU_CRITERION_SHADOW, GpuShapModelAlignedRaSEReducer, ShapAggregationConfig
...
def run(args: argparse.Namespace) -> pd.DataFrame:
    ...
    info = {
        "backend": "torch_cuda_model_aligned_with_shap_eta_probability_update",
        "torch": torch.__version__,
        "cuda_available": torch.cuda.is_available(),
        "device": args.device,
        "gpu_name": torch.cuda.get_device_name(0) if torch.cuda.is_available() else None,
        "criteria": criteria,
        "preserves": ["coord_prob", "eta_step", "SHAP-style attribution", "gamma_smoothing", "dynamic_bounds", "anchor_set", "box_simplex_projection", "eta_final"],
        **cfg_base,
    }
    ...
    for criterion in criteria:
        ...
        for fold, (tr, te) in enumerate(splits, 1):
            ...
            shap_cfg = ShapAggregationConfig(
                background_size=args.shap_background_size,
                eval_size=args.shap_eval_size,
                final_rbf_C=args.final_C,
                final_rbf_gamma=args.final_gamma,
            )
            reducer = GpuShapModelAlignedRaSEReducer(criterion, cfg, shap_cfg)
            t0 = perf_counter()
            ...
            t0 = perf_counter()
            model = GpuRbfKernelClassifier(C=args.final_C, gamma=args.final_gamma, device=args.device).fit(Ztr, y[tr])
                ...
                json.dumps(
                    {
                        **row,
                        ...
                        "iteration_history": reducer.iteration_history_,
                        "eta_final": reducer.eta_final_.astype(float).tolist(),
                    },
                    ensure_ascii=False,
                    indent=2,
                ),
            ...
            print(f"[done-gpu-shap] {criterion} fold={fold} acc={row['accuracy']:.4f} mcc={row['mcc']:.4f} auc={row['roc_auc']:.4f}", flush=True)
        if rows:
            pd.DataFrame(rows).to_csv(croot / "fold_metrics.csv", index=False)
            _summary(pd.DataFrame(rows)).to_csv(croot / "summary_metrics.csv", index=False)
...
def main() -> None:
    ...
    p.add_argument("--results-root", default="results_colon_gpu_shap_model_aligned_rase_q20_big_all6")
    ...
    p.add_argument("--shap-background-size", type=int, default=32)
    p.add_argument("--shap-eval-size", type=int, default=48)
    p.add_argument("--final-C", type=float, default=1.0)
