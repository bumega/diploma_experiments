...
def model_train(
    ...
    num_epochs: int,
    batch_size: int = 128,
    batch_size_pos: Optional[int] = None,
    batch_size_neg: Optional[int] = None,
    seed: int = 0,
    output: bool = False,
    ...
    if isinstance(train_data, TensorDataset):
        ...
        pos_bs = int(batch_size if batch_size_pos is None else batch_size_pos)
        neg_bs = int(batch_size if batch_size_neg is None else batch_size_neg)

        model_pos = train_one_model(
            model_pos, ds_pos, fmin, fmax,
            num_epochs=num_epochs, batch_size=pos_bs, seed=seed
        )
        model_neg = train_one_model(
            model_neg, ds_neg, fmin, fmax,
            num_epochs=num_epochs, batch_size=neg_bs, seed=seed + 1
        )
...
def metrics_from_sheet(X, y, model_pos, model_neg, beta_pos=0.1, beta_neg=0.1):
    ...
    # --- четыре зоны ---
    pos_hit = (s1 >= beta_pos) & (s2 < beta_neg)
    neg_hit = (s2 >= beta_neg) & (s1 < beta_pos)
    reject = (s1 < beta_pos) & (s2 < beta_neg)
    conflict = (s1 >= beta_pos) & (s2 >= beta_neg)

    # --- старые счетчики для F12 / G12 ---
    n02_1 = int((pos_hit & pos).sum().item())
    n12_1 = int((conflict & pos).sum().item())

    n02_2 = int((neg_hit & neg).sum().item())
    n12_2 = int((conflict & neg).sum().item())
    ...
    # --- новые метрики ---
    accepted = pos_hit | neg_hit
    n_accept = int(accepted.sum().item())
    n_reject = int(reject.sum().item())
    ...
    if n_accept > 0:
        selective_accuracy = float((y_pred[accepted] == y[accepted]).float().mean().item())
    else:
...
def make_dataset_rdt_waterspout(
    seed: int = 42,
    ...
    from rdt_dataset import load_rdt_waterspout

    loaded = load_rdt_waterspout(
        feature_set=feature_set,
        return_groups=return_groups,
        return_feature_names=return_feature_names,
        group_by=group_by,
    )
    if return_groups and return_feature_names:
        X, y, groups, feature_names = loaded
    elif return_groups:
        X, y, groups = loaded
        feature_names = None
        ...
        groups = None
        feature_names = None
    y = np.where(np.asarray(y).astype(int) > 0, 1, -1).astype(int)
    if return_groups and return_feature_names:
        return np.asarray(X, dtype=np.float64), y.astype(int), np.asarray(groups), feature_names
    if return_groups:
        return np.asarray(X, dtype=np.float64), y.astype(int), np.asarray(groups)
    if return_feature_names:
        return np.asarray(X, dtype=np.float64), y.astype(int), feature_names
    return np.asarray(X, dtype=np.float64), y.astype(int)
...
class HSICSelector(ReducerBase):
    ...
    def fit(self, X, y):
        ...
        self.model.classification(self.n_select, n_jobs=1)
...
def get_models(seed: int = 42) -> Dict[str, Any]:
    ...
    return {
        ...
        "LogRegL2": LogisticRegression(
            solver="lbfgs",   # стандартный solver для L2
            max_iter=5000,
            random_state=seed,
        ),
...
def _batched_train_with_checkpoints(
    ...
    batch_size_pos = int(batch_size if batch_size_pos is None else batch_size_pos)
    batch_size_neg = int(batch_size if batch_size_neg is None else batch_size_neg)
    ...
    def _train_one_epoch_one_side(
        ...
        for start in range(0, N_real, batch_size_side):
            idx = perm[:, start:start + batch_size_side]                 # [B, bs]
            cur_bs = int(idx.shape[1])
    ...
    for epoch in range(1, num_epochs + 1):
        _train_one_epoch_one_side(
            ...
            batch_size_side=batch_size_pos,
        )
        ...
        _train_one_epoch_one_side(
            ...
            batch_size_side=batch_size_neg,
        )
    ...
    for fold, (tr, te) in enumerate(skf.split(X, y_bin), 1):
        ...
        if compute_unary_on_reduced:
            ...
            model_pos, model_neg = model_train(
                ...
                batch_size_pos=unary_params.get("batch_size_pos"),
                batch_size_neg=unary_params.get("batch_size_neg"),
            ...
            unary_row = {
                ...
                "n1": int(m_un["n1"]),
                "n2": int(m_un["n2"]),
                "n_accept": int(m_un["n_accept"]),
                "n_reject": int(m_un["n_reject"]),
                "n_conflict": int(m_un["n_conflict"]),

                "coverage": float(m_un["coverage"]),
                "reject_rate": float(m_un["reject_rate"]),
                "conflict_rate": float(m_un["conflict_rate"]),
                "selective_accuracy": float(m_un["selective_accuracy"]),
...
class EnsembleReducer(ReducerBase):
    def __init__(
        ...
        self.num_epochs = num_epochs
        self.batch_size = batch_size
        self.batch_size_pos = int(batch_size if batch_size_pos is None else batch_size_pos)
        self.batch_size_neg = int(batch_size if batch_size_neg is None else batch_size_neg)
        self.seed = seed
        self.output = output
        self.n_select = n_select
        ...
        self.cG = float(cG)
        self.split_mode = self._normalize_split_mode(split_mode)
    ...
    def _normalize_split_mode(split_mode: str) -> str:
        aliases = {
            "independent": "split_60_20_20",
            "independent_splits": "split_60_20_20",
            "holdout": "split_60_20_20",
            "all": "full_data",
            "shared": "full_data",
            "same_data": "full_data",
            "cv": "cv_5fold",
        ...
        mode = aliases.get(str(split_mode), str(split_mode))
        allowed = {"split_60_20_20", "full_data", "cv_5fold"}
        if mode not in allowed:
            raise ValueError(
                f"Unknown split_mode={split_mode!r}. Allowed values: {sorted(allowed)}"
            )
        return mode
    ...
    def _make_holdout_splits(self, idx_np: np.ndarray, y_np: np.ndarray):
        if self.split_mode == "split_60_20_20":
        ...
        elif self.split_mode == "full_data":
            tr_idx_np = idx_np.copy()
            par_idx_np = idx_np.copy()
            val_idx_np = idx_np.copy()
        else:
            raise ValueError(
                f"_make_holdout_splits is not defined for split_mode={self.split_mode}"
            )

        return tr_idx_np, par_idx_np, val_idx_np
    ...
    def _make_cv_folds(
        ...
        max_possible_splits = int(class_counts.min())
        if max_possible_splits < 2:
            raise ValueError("Need at least 2 samples in each class for cv_5fold mode.")

        n_splits = min(self.cv_n_splits, max_possible_splits)
        ...
        dummy_X = np.zeros((len(idx_np), 1), dtype=np.float32)
        return [(idx_np[tr], idx_np[val]) for tr, val in skf.split(dummy_X, y_np)]

    def _build_shap_sampling_indices(
        self,
        X_tr: torch.Tensor,
        X_par: torch.Tensor,
        y_par: torch.Tensor,
        ...
        return bg_idx, ev_idx

    def _train_and_score_chunk(
        self,
        *,
        subs_batch_np: np.ndarray,
        X_train: torch.Tensor,
        y_train: torch.Tensor,
        X_par_ckpt: torch.Tensor,
        y_par_ckpt: torch.Tensor,
        X_val_eval: torch.Tensor,
        y_val_eval: torch.Tensor,
        fmin_all: torch.Tensor,
        fmax_all: torch.Tensor,
        k: int,
        i: int,
        d_cur: int,
        chunk_id: int,
        ...
        X_sub_tr_batch = _gather_subspaces_2d(X_train, subs_batch_t)
        X_sub_par_batch = _gather_subspaces_2d(X_par_ckpt, subs_batch_t)
        X_sub_val_batch = _gather_subspaces_2d(X_val_eval, subs_batch_t)

        fmin_batch = fmin_all[subs_batch_t]
        fmax_batch = fmax_all[subs_batch_t]

        pos_tr_mask = (y_train == 1)
        neg_tr_mask = (y_train == -1)
        ...
        ckpt = _batched_train_with_checkpoints(
            ...
            num_epochs=self.num_epochs,
            batch_size=self.batch_size,
            batch_size_pos=self.batch_size_pos,
        ...
        return {
            "ckpt": ckpt,
            "subs_batch_np": subs_batch_np,
            "beta_pos": beta_pos_batch.detach().cpu().numpy(),
            "beta_neg": beta_neg_batch.detach().cpu().numpy(),
            "F12_val": F12_val_batch.detach().cpu().numpy(),
            "G12_val": G12_val_batch.detach().cpu().numpy(),
            "S_val": S_val_batch.detach().cpu().numpy(),
        }

    def _select_best_subspace_single_split(
        self,
        *,
        n_feat: int,
        X_tr: torch.Tensor,
        ...
        for subs_idx in candidates:
            groups[len(subs_idx)].append(subs_idx)

        for d_cur, subs_list in groups.items():
            for chunk_id, items_chunk in enumerate(
                _chunk_list(subs_list, self.max_models_per_batch)
            ):
                subs_batch_np = np.stack(items_chunk, axis=0)

                res = self._train_and_score_chunk(
                    subs_batch_np=subs_batch_np,
                    X_train=X_tr,
                    y_train=y_tr,
                    X_par_ckpt=X_par,
                ...
                loc = int(np.argmax(res["S_val"]))
                S_cur = float(res["S_val"][loc])

                if S_cur > best["score"]:
                    ckpt = res["ckpt"]

                    model_pos = BatchedMLP.build_model_from_state(
                        ckpt["best_pos_states"][loc],
                        device=self.device,
                    )
                    model_neg = BatchedMLP.build_model_from_state(
                    ...
                    best.update(
                        {
                            "score": S_cur,
                            "subs_idx": subs_batch_np[loc].copy(),
                            "beta_pos": float(res["beta_pos"][loc]),
                            "beta_neg": float(res["beta_neg"][loc]),
                            "epoch": int(ckpt["best_epoch"][loc]),
                            "S_par": float(ckpt["best_S_par"][loc]),
                            "F12_par": float(ckpt["best_F12_par"][loc]),
                            "G12_par": float(ckpt["best_G12_par"][loc]),
                            "F12_val": float(res["F12_val"][loc]),
                            "G12_val": float(res["G12_val"][loc]),
                            "model_pos": model_pos,
                            "model_neg": model_neg,
                        }
                    )
    ...
    def _retrain_best_subspace_on_full(
        self,
        *,
        X_full: torch.Tensor,
        y_full: torch.Tensor,
        best_subs_idx: np.ndarray,
        k: int,
        i: int,
    ) -> dict:
        fmin_all, fmax_all = compute_feature_bounds(TensorDataset(X_full, y_full))
        fmin_all = fmin_all.to(device=X_full.device, dtype=X_full.dtype)
        fmax_all = fmax_all.to(device=X_full.device, dtype=X_full.dtype)

        res = self._train_and_score_chunk(
            subs_batch_np=np.asarray(best_subs_idx, dtype=int)[None, :],
            X_train=X_full,
            y_train=y_full,
            X_par_ckpt=X_full,
            y_par_ckpt=y_full,
            ...
            k=k,
            i=i,
            d_cur=len(best_subs_idx),
            chunk_id=0,
            fold_id=self.cv_n_splits + 1,
        )

        ckpt = res["ckpt"]

        model_pos = BatchedMLP.build_model_from_state(
            ckpt["best_pos_states"][0],
            device=self.device,
        )
        model_neg = BatchedMLP.build_model_from_state(
            ckpt["best_neg_states"][0],
            device=self.device,
        )
        model_pos.eval()
        model_neg.eval()
        ...
        return {
            "model_pos": model_pos,
            "model_neg": model_neg,
            "beta_pos": float(res["beta_pos"][0]),
            "beta_neg": float(res["beta_neg"][0]),
            "epoch": int(ckpt["best_epoch"][0]),
            "S_par": float(ckpt["best_S_par"][0]),
            "F12_par": float(ckpt["best_F12_par"][0]),
            "G12_par": float(ckpt["best_G12_par"][0]),
            "F12_full": float(res["F12_val"][0]),
            "G12_full": float(res["G12_val"][0]),
            "S_full": float(res["S_val"][0]),
        }

    def _select_best_subspace_cv(
        self,
        *,
        n_feat: int,
        X_full: torch.Tensor,
        y_full: torch.Tensor,
        cv_folds_np: list[tuple[np.ndarray, np.ndarray]],
        k: int,
        i: int,
    ) -> dict:
        best = {
        ...
        for d_cur, subs_list in groups.items():
            for chunk_id, items_chunk in enumerate(
                ...
                subs_batch_np = np.stack(items_chunk, axis=0)
                s_sum = np.zeros(len(items_chunk), dtype=float)
                f_sum = np.zeros(len(items_chunk), dtype=float)
                g_sum = np.zeros(len(items_chunk), dtype=float)

                for fold_id, (tr_fold_np, val_fold_np) in enumerate(cv_folds_np):
                    tr_fold = torch.as_tensor(
                        tr_fold_np, dtype=torch.long, device=X_full.device
                    )
                    val_fold = torch.as_tensor(
                        val_fold_np, dtype=torch.long, device=X_full.device
                    )

                    X_tr_fold, y_tr_fold = X_full[tr_fold], y_full[tr_fold]
                    X_val_fold, y_val_fold = X_full[val_fold], y_full[val_fold]

                    fmin_all, fmax_all = compute_feature_bounds(
                        TensorDataset(X_tr_fold, y_tr_fold)
                    )
                    fmin_all = fmin_all.to(device=X_tr_fold.device, dtype=X_tr_fold.dtype)
                    fmax_all = fmax_all.to(device=X_tr_fold.device, dtype=X_tr_fold.dtype)

                    res = self._train_and_score_chunk(
                        subs_batch_np=subs_batch_np,
                        X_train=X_tr_fold,
                        y_train=y_tr_fold,
                        X_par_ckpt=X_tr_fold,
                        y_par_ckpt=y_tr_fold,
                        X_val_eval=X_val_fold,
                        y_val_eval=y_val_fold,
                        fmin_all=fmin_all,
                        fmax_all=fmax_all,
                        k=k,
                        i=i,
                ...
                n_folds = float(len(cv_folds_np))
                s_mean = s_sum / n_folds
                f_mean = f_sum / n_folds
                g_mean = g_sum / n_folds

                loc = int(np.argmax(s_mean))
                S_cur = float(s_mean[loc])
                    ...
                    best.update(
                        {
                            "score": S_cur,
                            "subs_idx": subs_batch_np[loc].copy(),
                            "F12_cv": float(f_mean[loc]),
                            "G12_cv": float(g_mean[loc]),
                        }
                    )

        retrained = self._retrain_best_subspace_on_full(
            X_full=X_full,
            y_full=y_full,
            best_subs_idx=best["subs_idx"],
            k=k,
            i=i,
        )
        ...
        return {
            "score": best["score"],
            "subs_idx": best["subs_idx"],
            "beta_pos": retrained["beta_pos"],
            "beta_neg": retrained["beta_neg"],
            "epoch": retrained["epoch"],
            "S_par": retrained["S_par"],
            "F12_par": retrained["F12_par"],
            "G12_par": retrained["G12_par"],
            "F12_val": best["F12_cv"],
            "G12_val": best["G12_cv"],
            "model_pos": retrained["model_pos"],
            "model_neg": retrained["model_neg"],
            "S_full": retrained["S_full"],
            "F12_full": retrained["F12_full"],
            "G12_full": retrained["G12_full"],
        }
    ...
    def _compute_step_shap_batched(
        ...
        if self.use_gpu_shap and self.device.type == "cuda":
            ...
            X_par_src = X_par
            y_par_src = y_par
            bg_idx_src = bg_idx.to(device=X_tr_src.device)
            ev_idx_src = ev_idx.to(device=X_par_src.device)
        else:
            shap_device = torch.device("cpu")
            X_tr_src = X_tr.detach().cpu()
            X_par_src = X_par.detach().cpu()
            y_par_src = y_par.detach().cpu()
            bg_idx_src = bg_idx.detach().cpu()
            ev_idx_src = ev_idx.detach().cpu()

        X_bg_base = X_tr_src[bg_idx_src]
        X_eval_base = X_par_src[ev_idx_src]
        y_eval_base = y_par_src[ev_idx_src]
    ...
    def fit(self, X, y):
        ...
        if self.split_mode == "cv_5fold":
            cv_folds_np = self._make_cv_folds(idx_np, y_np)

            if self.output:
                fold_pos_counts = [int((y_np[tr_fold_np] == 1).sum()) for tr_fold_np, _ in cv_folds_np]
                fold_neg_counts = [int((y_np[tr_fold_np] == -1).sum()) for tr_fold_np, _ in cv_folds_np]
                fold_pos_steps = [int(np.ceil(max(1, n_pos) / self.batch_size_pos)) for n_pos in fold_pos_counts]
                fold_neg_steps = [int(np.ceil(max(1, n_neg) / self.batch_size_neg)) for n_neg in fold_neg_counts]
                print(
                    f"[ensemble-train-config] split_mode={self.split_mode} "
                    f"batch_size_pos={self.batch_size_pos} batch_size_neg={self.batch_size_neg} "
                    f"train_pos_range={min(fold_pos_counts)}..{max(fold_pos_counts)} "
                    f"train_neg_range={min(fold_neg_counts)}..{max(fold_neg_counts)} "
                    f"pos_steps_per_epoch_range={min(fold_pos_steps)}..{max(fold_pos_steps)} "
                    f"neg_steps_per_epoch_range={min(fold_neg_steps)}..{max(fold_neg_steps)}"
                )

            X_shap_tr, y_shap_tr = X_t, y_t
            X_shap_par, y_shap_par = X_t, y_t
            bg_idx, ev_idx = self._build_shap_sampling_indices(
                X_shap_tr, X_shap_par, y_shap_par
            )
        else:
            tr_idx_np, par_idx_np, val_idx_np = self._make_holdout_splits(idx_np, y_np)
            ...
            if self.output:
                ...
                n_neg_tr = int((y_tr == -1).sum().item())
                pos_steps = int(np.ceil(max(1, n_pos_tr) / self.batch_size_pos))
                neg_steps = int(np.ceil(max(1, n_neg_tr) / self.batch_size_neg))
                print(
                    f"[ensemble-train-config] split_mode={self.split_mode} "
                    f"train_pos={n_pos_tr} train_neg={n_neg_tr} "
                    f"batch_size_pos={self.batch_size_pos} batch_size_neg={self.batch_size_neg} "
                    f"pos_steps_per_epoch={pos_steps} neg_steps_per_epoch={neg_steps}"
                )
            ...
            X_shap_tr, y_shap_tr = X_tr, y_tr
            X_shap_par, y_shap_par = X_par, y_par
            bg_idx, ev_idx = self._build_shap_sampling_indices(
                X_shap_tr, X_shap_par, y_shap_par
            )
        ...
        for k in range(self.num_samples):
            for i in range(self.num_models):
                if self.split_mode == "cv_5fold":
                    best = self._select_best_subspace_cv(
                        n_feat=n_feat,
                        X_full=X_t,
                        y_full=y_t,
                        cv_folds_np=cv_folds_np,
                        k=k,
                        i=i,
                    )
                else:
                    best = self._select_best_subspace_single_split(
                        n_feat=n_feat,
                        X_tr=X_tr,
                        y_tr=y_tr,
                        X_par=X_par,
                ...
                self.list_model_pos.append(best["model_pos"])
                self.list_model_neg.append(best["model_neg"])
                self.list_model_subs.append(best["subs_idx"])
                self.list_beta_pos.append(best["beta_pos"])
                self.list_beta_neg.append(best["beta_neg"])
                self.list_best_S.append(best["score"])

                self.list_best_epoch.append(best["epoch"])
                self.list_best_S_par.append(best["S_par"])
                self.list_best_F12_par.append(best["F12_par"])
                self.list_best_G12_par.append(best["G12_par"])
                self.list_best_F12_val.append(best["F12_val"])
                self.list_best_G12_val.append(best["G12_val"])

                if self.output:
                    if self.split_mode == "cv_5fold":
                        print(
                            f"[outer={k+1}/{self.num_samples} model={i+1}/{self.num_models}] "
                            f"d={len(best['subs_idx']) if best['subs_idx'] is not None else -1} "
                            f"epoch_best={best['epoch']} "
                            f"S_cv={best['score']:.6f} "
                            f"F12_cv={best['F12_val']:.6f} "
                            f"G12_cv={best['G12_val']:.6f} "
                            f"S_full={best['S_full']:.6f} "
                            f"F12_full={best['F12_full']:.6f} "
                            f"G12_full={best['G12_full']:.6f} "
                            f"beta_pos={best['beta_pos']:.6f} "
                            f"beta_neg={best['beta_neg']:.6f} "
                            f"cG={self.cG:.6f}"
                        )
                    else:
                        print(
                            ...
                            f"d={len(best['subs_idx']) if best['subs_idx'] is not None else -1} "
                            f"epoch_best={best['epoch']} "
                            f"S_par={best['S_par']:.6f} "
                            f"F12_par={best['F12_par']:.6f} "
                            f"G12_par={best['G12_par']:.6f} "
                            f"S_val={best['score']:.6f} "
                            f"F12_val={best['F12_val']:.6f} "
                            f"G12_val={best['G12_val']:.6f} "
                            f"beta_pos={best['beta_pos']:.6f} "
                            f"beta_neg={best['beta_neg']:.6f} "
                            f"cG={self.cG:.6f}"
                        )
...
def run_colon_10fold_cv(
    ...
    for fold_id, (train_idx, test_idx) in enumerate(skf.split(X, y), start=1):
        ...
        clf = LogisticRegression(
            l1_ratio=1.0,
            solver="saga",             # корректная связка с l1_ratio
            C=1.0,
            max_iter=5000,
...
def train_unary_pair_three_splits(
    ...
    *,
    ...
    pos_steps = int(np.ceil(max(1, X_pos_fit.shape[0]) / batch_size_pos))
    neg_steps = int(np.ceil(max(1, X_neg_fit.shape[0]) / batch_size_neg))
    print(
        f"[unary-train-config] train_pos={int(X_pos_fit.shape[0])} train_neg={int(X_neg_fit.shape[0])} "
        f"batch_size_pos={batch_size_pos} batch_size_neg={batch_size_neg} "
        f"pos_steps_per_epoch={pos_steps} neg_steps_per_epoch={neg_steps}"
    )
...
def run_mlp_on_saved_folds(
    results_dir: str,
    *,
    hidden_layer_sizes: tuple[int, ...] = (64, 32),
    activation: str = "relu",          # ReLU = кусочно-линейная активация
    solver: str = "adam",
    alpha: float = 1e-4,
    ...
    tol: float = 1e-4,
    seed: int = 42,
    save_name: str = "mlp_results.csv",
) -> pd.DataFrame:
    """
    Запуск по всем сохранённым фолдам для обычного MLP-классификатора.

    Что значит "обучать до переобучения":
    - модель обучается долго (max_iter большой),
    - но с early_stopping=True,
    - на каждом train-fold внутри берётся внутренняя validation-часть,
    - если качество на ней перестаёт улучшаться, обучение останавливается.

    Это наиболее близкий аналог "учить, пока не начнёт переобучаться"
    в рамках sklearn MLPClassifier.

    Важно:
    - activation='relu' даёт кусочно-линейную активацию;
    - масштабирование уже делается внутри общего Pipeline в
      _run_classifier_on_saved_folds, поэтому утечки нет.
    """
    if activation != "relu":
        raise ValueError(
            "Для кусочно-линейной активации здесь ожидается activation='relu'."
        )

    def build_model(fold_seed: int) -> MLPClassifier:
